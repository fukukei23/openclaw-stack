...
The fopenclaw.com environment hosts a production OpenClaw Gateway on a VPS (162.43.17.111) protected by Caddy for TLS, HTTP BasicAuth, and reverse proxying into a loopback-bound gateway (`127.0.0.1:18789`). Device pairing is complete, a gateway token `<REDACTED>` is loaded via secrets, and all agent traffic uses the `openai/gpt-5.1-codex` model. Health checks are green, but the setup assumes strict firewall enforcement (UFW allowing only 80/443 and throttled 22).

## Environment Summary
- **Domain:** `fopenclaw.com` → `162.43.17.111`
